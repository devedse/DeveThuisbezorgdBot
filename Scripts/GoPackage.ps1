...
$7zFilePath = Join-Path $outputDir "DeveThuisbezorgdBot.7z"
$zipFilePath = Join-Path $outputDir "DeveThuisbezorgdBot.zip"

DeleteFileIfExists $7zFilePath
DeleteFileIfExists $zipFilePath
DeleteFolderIfExists $outputDir

$buildPath = Join-Path $solutionRoot "DeveThuisbezorgdBot\bin\Release\netstandard2.0"
